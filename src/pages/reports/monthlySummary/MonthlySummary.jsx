...
function MonthlySummary() {
  ...
  const exportTableToPDF = () => {
    const table = document.getElementById('MonthlySummary');
  ...
  const resetReport = () => {
    window.location.href = '/reports/monthlySummary';
  };

  const summaryCards = useMemo(() => ([
    { title: 'Total Sales', value: 'Rs. 14,300.00', note: '2 transactions', bg: 'bg-blue-100', titleColor: 'text-blue-800', valueColor: 'text-blue-900', noteColor: 'text-blue-700' },
    ...
    { title: 'Net Sales', value: 'Rs. 15,600.00', note: 'After returns deduction', bg: 'bg-green-100', titleColor: 'text-green-800', valueColor: 'text-green-900', noteColor: 'text-green-700' },
    { title: 'Total Cost', value: 'Rs. 13,000.00', bg: 'bg-blue-100', titleColor: 'text-blue-800', valueColor: 'text-blue-900' },
    { title: 'Revenue Before Discount', value: 'Rs. 15,600.00', bg: 'bg-orange-100', titleColor: 'text-orange-800', valueColor: 'text-orange-900' },
    ...
    { title: 'Gross Revenue', value: 'Rs. 15,600.00', note: 'Before returns', bg: 'bg-green-100', titleColor: 'text-green-800', valueColor: 'text-green-900', noteColor: 'text-green-700' },
    { title: 'Net Profit', value: 'Rs. 2,600.00', note: 'After returns', bg: 'bg-yellow-100', titleColor: 'text-yellow-800', valueColor: 'text-yellow-900', noteColor: 'text-yellow-700' },
    { title: 'Net Margin', value: '16.67%', note: 'Net profit margin', bg: 'bg-purple-100', titleColor: 'text-purple-800', valueColor: 'text-purple-900', noteColor: 'text-purple-700' },
  ]), []);

  return (
    <div className="monthly-summary-page min-h-dvh max-lg:h-fit flex flex-col h-dvh">
          ...
          <ol className="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
            ...
            <li aria-current="page">
              <div className="flex items-center">
                ...
                <p className="text-sm font-medium text-gray-700 ms-1 md:ms-2">Monthly Summary Report</p>
              </div>
            </li>
          </ol>
      ...
      <div className="px-12 max-sm:px-6">
        <form onSubmit={(event) => {
          ...
          <div className="grid gap-4 py-4 md:grid-cols-5">
            <div>
              <label htmlFor="month" className="block mb-1 text-xs font-medium text-black">Month</label>
              <input id="month" name="month" type="month" value={month} onChange={(event) => setMonth(event.target.value)} className="w-full p-3 text-xs text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500" />
            </div>
            <div>
      ...
      <div className="flex flex-col px-12 py-5 overflow-y-auto bg-white max-sm:px-6 max-lg:min-h-full">
        <div className="relative h-[500px] overflow-x-auto">
          <table id="MonthlySummary" className="w-full text-sm text-left text-gray-500">
            ...
            <tbody className="space-y-2">
              {monthlyRows.map((row) => (
                <tr key={row.no} className="bg-white">
                  ...
                  <td className="px-3 py-2 text-right text-green-600 font-bold">{row.netProfit}</td>
                  <td className="px-3 py-2 text-right text-green-600">{row.profitPercent}</td>
                </tr>
              ))}
            </tbody>
          </table>
