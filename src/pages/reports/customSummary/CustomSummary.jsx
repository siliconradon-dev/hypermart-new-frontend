...
function CustomSummary() {
  ...
  const [fromDate, setFromDate] = useState('2026-04-06T00:00');
  const [toDate, setToDate] = useState('2026-04-06T23:59');
  ...
  const exportTableToPDF = () => {
    const table = document.getElementById('CustomSummary');

    if (!table) {
      // Keep the same failure mode as the HTML page, which assumes the table exists.
      return;
    }
  ...
  const resetReport = () => {
    window.location.href = '/reports/customSummary';
  };

  const summaryCards = useMemo(() => ([
    { title: 'Total Sales', value: 'Rs. 0.00', note: '0 transactions', bg: 'bg-blue-100', titleColor: 'text-blue-800', valueColor: 'text-blue-900', noteColor: 'text-blue-700' },
    { title: 'Total Returns', value: 'Rs. 0.00', note: '0 items returned', bg: 'bg-red-100', titleColor: 'text-red-800', valueColor: 'text-red-900', noteColor: 'text-red-700' },
    ...
    { title: 'Net Sales', value: 'Rs. 0.00', note: 'After returns deduction', bg: 'bg-green-100', titleColor: 'text-green-800', valueColor: 'text-green-900', noteColor: 'text-green-700' },
    { title: 'Total Cost', value: 'Rs. 0.00', bg: 'bg-blue-100', titleColor: 'text-blue-800', valueColor: 'text-blue-900' },
    { title: 'Revenue Before Discount', value: 'Rs. 0.00', bg: 'bg-orange-100', titleColor: 'text-orange-800', valueColor: 'text-orange-900' },
    { title: 'Total Discounts', value: 'Rs. 0.00', note: '0.00%', bg: 'bg-red-100', titleColor: 'text-red-800', valueColor: 'text-red-900', noteColor: 'text-red-700' },
    { title: 'Gross Revenue', value: 'Rs. 0.00', note: 'Before returns', bg: 'bg-green-100', titleColor: 'text-green-800', valueColor: 'text-green-900', noteColor: 'text-green-700' },
    { title: 'Net Profit', value: 'Rs. 0.00', note: 'After returns', bg: 'bg-yellow-100', titleColor: 'text-yellow-800', valueColor: 'text-yellow-900', noteColor: 'text-yellow-700' },
    { title: 'Net Margin', value: '0.00%', note: 'Net profit margin', bg: 'bg-purple-100', titleColor: 'text-purple-800', valueColor: 'text-purple-900', noteColor: 'text-purple-700' },
  ]), []);

  return (
    <div className="custom-summary-page min-h-dvh max-lg:h-fit flex flex-col h-dvh">
      ...
      <div style={{ display: 'none' }} data-company-name="Hypermart" />
          ...
          <ol className="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
            ...
            <li aria-current="page">
              <div className="flex items-center">
                ...
                <p className="text-sm font-medium text-gray-700 ms-1 md:ms-2">Custom Summary Report</p>
              </div>
            </li>
          </ol>
      ...
      <div className="px-12 max-sm:px-6">
        <form onSubmit={(event) => {
          ...
          <div className="grid gap-4 py-4 md:grid-cols-4">
            <div>
              <label htmlFor="from_date" className="block mb-1 text-xs font-medium text-black">From Date &amp; Time</label>
              <input
                id="from_date"
                name="from_date"
                type="datetime-local"
            ...
            <div>
              <label htmlFor="to_date" className="block mb-1 text-xs font-medium text-black">To Date &amp; Time</label>
              <input
                id="to_date"
                name="to_date"
                type="datetime-local"
            ...
            <div>
              ...
              <div className="flex w-full gap-3 custom-select">
                <select id="items_id" name="items_id" value={itemsId} onChange={(event) => setItemsId(event.target.value)} className="hidden">
            ...
            <div className="flex justify-start gap-2 md:col-span-1">
              <button type="submit" className="py-0.5 px-3 w-auto bg-[#3c8c2c] text-white rounded-lg text-lg">Filter</button>
              <button type="button" className="py-0.5 px-3 w-auto bg-[#000000] text-white rounded-lg text-lg" onClick={resetReport}>Reset</button>
            </div>
          </div>
        </form>
      </div>

      <div className="px-12 pb-0 max-sm:px-6">
      ...
      <div className="flex flex-col px-12 py-5 overflow-y-auto bg-white max-sm:px-6 max-lg:min-h-full">
        <div className="relative h-[500px] overflow-x-auto">
          <table id="CustomSummary" className="w-full text-sm text-left text-gray-500">
              ...
              {summaryRows.length > 0 ? null : (
                <tr>
                  <td colSpan={14} className="px-3 py-8 text-center text-gray-500">
                    No sales data found for this time period
                  </td>
                </tr>
              )}
            </tbody>
